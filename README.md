...
```mermaid
flowchart LR
    A[📱 User Movement / Sensors] --> B[🤖 AI Prediction Model]
    B --> C{⚠️ Danger Detected?}
    C -- Yes --> D[🔔 Send Alert to User]
    C -- Yes --> E[📞 Notify Trusted Contacts]
    C -- No --> F[🔄 Monitor Continuously]
```
